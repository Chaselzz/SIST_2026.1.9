...
# Init GLFW
glfw.init()
window = glfw.create_window(1920, 1080, "UR5e 平滑曲线书写仿真（优化间距）", None, None)
...
while not glfw.window_should_close(window):
    ...
    while (data.time - time_prev < 1.0/60.0):
        ...
        if data.time >= t_total and not post_writing_lifted:
            ...
            # 检查是否已经到达悬停高度（加一个小余量）
            if current_pos[2] >= LIFT_HEIGHT - 0.01:
                print(f"【抬笔悬停完成】{data.time:.2f}s 机械臂已悬停在高度 {current_pos[2]:.3f}m")
            ...
            # 打印抬笔悬停进度
                ...
                print(f"【抬笔悬停中】{data.time:.2f}s 当前高度 {current_pos[2]:.3f}m，目标高度 {LIFT_HEIGHT}m")
...
messagebox.showinfo("仿真完成", 
                   f"字符串: '{input_string}'\n"
                   f"仿真时间: {data.time:.2f}秒\n"
                   f"轨迹点数: {len(traj_points)}\n"
                   f"缩放因子: {SCALE_FACTOR} (扩大2倍)\n"
                   f"起始位置: X={WRITING_AREA_X_OFFSET}, Y={WRITING_AREA_Y_OFFSET}\n"
                   f"字符间距: {CHAR_SPACING * SCALE_FACTOR:.3f} (无空格时)\n"
                   f"空格大小: {AVERAGE_CHAR_WIDTH * SCALE_FACTOR:.3f} (1个字符宽度)\n"
                   f"悬停高度: {LIFT_HEIGHT}m (完成书写后抬笔悬停)\n"
                   f"插值方式: {interpolate_type}\n\n"
                   "仿真已完成！")
